  ...
  fun signIn(ctx: RoutingContext) {
    ctx.request().setExpectMultipart(true).endHandler {
      ...
      if (user == null || !BCrypt.checkpw(password, user.hashedPassword)) {
        ctx.response().setContentType(ContentType.TEXT_PLAIN).setStatusCode(401)
  ...
  fun signUp(ctx: RoutingContext) {
    ctx.request().setExpectMultipart(true).endHandler {
      ...
      if (existingUser != null) {
        ctx.response().setContentType(ContentType.TEXT_PLAIN).setStatusCode(409)
      ...
      val session = sessionsRepository.createSession(user.id)
      val cookie = Cookie.cookie("session_token", session.token).setPath("/")
        .setMaxAge(3600).setSecure(true).setHttpOnly(true)
      ctx.response().addCookie(cookie).redirect("/")
      return@endHandler
    }
  }
