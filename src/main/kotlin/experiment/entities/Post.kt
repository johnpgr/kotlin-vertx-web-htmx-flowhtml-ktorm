...
object Posts : Table<Post>("post") {
  ...
  val authorId = uuid("author_id").bindTo {
    it.authorId
  }.references(Users) {
    it.author
  }
  val slug = varchar("slug").bindTo {
    it.slug
  }
  val title = varchar("title").bindTo {
    it.title
  }
  val description = varchar("description").bindTo {
    it.description
  }
  val body = varchar("body").bindTo {
    it.body
  }
  val createdAt = datetime("created_at").bindTo {
    it.createdAt
  }
  val updatedAt = datetime("updated_at").bindTo {
    it.updatedAt
  }
}
